...
pmn=16*q0/pi^2./(m*n'); %Loat mag per Fourier
p=sin(x*pi*m'/a)*pmn*sin(n*pi*y'/b); %load

wmn=pmn./(D*pi^4*(repmat(m,[1 length(n)]).^2/a^2+repmat(n',[length(m) 1]).^2/b^2).^2);%Displacements mag per Fourier
w=sin(x*pi*m'/a)*wmn*sin(n*pi*y'/b);%Displacemnet
...
%% Store output
[AP.x_mesh,AP.y_mesh]=meshgrid(x,y); %Conveniant for plotting
